...
class PathConstraints:
    ...
    def __eq__(self, other):
        return (
            isinstance(other, PathConstraints)
...
class ConstraintsZone:
    ...
    def forSegmentIndex(self, segment_index: int) -> ConstraintsZone:
        ...
        return ConstraintsZone(
            self.minWaypointPos - segment_index,
            self.maxWaypointPos - segment_index,
            self.constraints,
        )

    def __eq__(self, other):
        return (
            isinstance(other, ConstraintsZone)
...
class RotationTarget:
    ...
    def forSegmentIndex(self, segment_index: int) -> RotationTarget:
        ...
        return RotationTarget(
            self.waypointRelativePosition - segment_index, self.target, self.rotateFast
        )

    def __eq__(self, other):
        return (
            isinstance(other, RotationTarget)
    ...
    def __init__(
        ...
        for t in decimal_range(0.0, 1.0, RESOLUTION):
            ...
            if len(target_holonomic_rotations) > 0:
                if math.fabs(
                    target_holonomic_rotations[0].waypointRelativePosition - t
                ) <= math.fabs(
                    target_holonomic_rotations[0].waypointRelativePosition
                    - min(t + RESOLUTION, 1.0)
                ):
            ...
            if currentZone is not None:
                self.segmentPoints.append(
                    PathPoint(
                        cubicLerp(p1, p2, p3, p4, t),
                        holonomicRotation,
                        currentZone.constraints,
                    )
                )
            else:
                self.segmentPoints.append(
                    PathPoint(cubicLerp(p1, p2, p3, p4, t), holonomicRotation)
                )

        if end_segment:
            holonomicRotation = (
                target_holonomic_rotations.pop(0)
                if len(target_holonomic_rotations) > 0
                else None
            )
            self.segmentPoints.append(
                PathPoint(cubicLerp(p1, p2, p3, p4, 1.0), holonomicRotation)
            )

    @staticmethod
    def _findConstraintsZone(
        zones: List[ConstraintsZone], t: float
    ) -> ConstraintsZone | None:
        for zone in zones:
...
class PathPlannerPath:
    ...
    def __init__(
        ...
        if len(bezier_points) >= 4:
            self._allPoints = PathPlannerPath._createPath(
                self._bezierPoints, self._rotationTargets, self._constraintZones
            )
            self._precalcValues()
        ...
        report(
            tResourceType.kResourceType_PathPlannerPath.value,
            PathPlannerPath._instances,
        )
    ...
    def fromPathFile(path_name: str) -> PathPlannerPath:
        ...
        filePath = os.path.join(
            getDeployDirectory(), "pathplanner", "paths", path_name + ".path"
        )
    ...
    def fromChoreoTrajectory(trajectory_name: str) -> PathPlannerPath:
        ...
        """
        filePath = os.path.join(
            getDeployDirectory(), "choreo", trajectory_name + ".traj"
        )
        ...
        with open(filePath, "r") as f:
            ...
            for s in trajJson["samples"]:
                ...
                state.headingAngularVelocityRps = (
                    0.0  # Not encoded, only used for diff drive anyway
                )
            ...
            path = PathPlannerPath(
                [],
                PathConstraints(float("inf"), float("inf"), float("inf"), float("inf")),
                GoalEndState(
                    trajStates[-1].velocityMps,
                    trajStates[-1].targetHolonomicRotation,
                    True,
                ),
            )
            ...
            path._choreoTrajectory = PathPlannerTrajectory(
                None, None, None, states=trajStates, event_commands=eventCommands
            )
    ...
    def bezierFromPoses(poses: List[Pose2d]) -> List[Translation2d]:
        ...
        for i in range(1, len(poses) - 1):
            ...
            bezierPoints.append(
                anchor
                + Translation2d(
                    anchor.distance(poses[i - 1].translation()) / 3.0,
        ...
        # Last pose
        bezierPoints.append(
            poses[len(poses) - 1].translation()
            + Translation2d(
                poses[len(poses) - 1]
                .translation()
                .distance(poses[len(poses) - 2].translation())
                / 3.0,
    ...
    def getStartingDifferentialPose(self) -> Pose2d:
        ...
        if self._reversed:
            heading = Rotation2d.fromDegrees(
                inputModulus(heading.degrees() + 180, -180, 180)
            )
    ...
    def getPreviewStartingHolonomicPose(self) -> Pose2d:
        ...
        heading = (
            Rotation2d()
            if self._previewStartingRotation is None
            else self._previewStartingRotation
        )
    ...
    def replan(
        ...
        currentFieldRelativeSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(
            current_speeds.vx,
            current_speeds.vy,
        ...
        linearVel = math.hypot(
            currentFieldRelativeSpeeds.vx, currentFieldRelativeSpeeds.vy
        )
        if linearVel > 0.1:
            stoppingDistance = (linearVel**2) / (
                2 * self._globalConstraints.maxAccelerationMpsSq
            )

            heading = Rotation2d(
                currentFieldRelativeSpeeds.vx, currentFieldRelativeSpeeds.vy
            )
            robotNextControl = starting_pose.translation() + Translation2d(
                stoppingDistance / 2.0, heading
            )

        closestPointIdx = 0
        comparePoint = (
            robotNextControl
            if robotNextControl is not None
            else starting_pose.translation()
        )
        closestDist = PathPlannerPath._positionDelta(
            comparePoint, self.getPoint(closestPointIdx).position
        )
        ...
        if closestPointIdx == self.numPoints() - 1:
            heading = (
                self.getPoint(self.numPoints() - 1).position - comparePoint
            ).angle()

            if robotNextControl is None:
                robotNextControl = starting_pose.translation() + Translation2d(
                    closestDist / 3.0, heading
                )

            endPrevControlHeading = (
                self.getPoint(self.numPoints() - 1).position - robotNextControl
            ).angle()

            endPrevControl = self.getPoint(
                self.numPoints() - 1
            ).position - Translation2d(closestDist / 3.0, endPrevControlHeading)
        ...
        elif (closestPointIdx == 0 and robotNextControl is None) or (
            math.fabs(
                closestDist
                - starting_pose.translation().distance(self.getPoint(0).position)
            )
            <= 0.25
            and linearVel < 0.1
        ):
            distToStart = starting_pose.translation().distance(
                self.getPoint(0).position
            )

            heading = (self.getPoint(0).position - starting_pose.translation()).angle()
            robotNextControl = starting_pose.translation() + Translation2d(
                distToStart / 3.0, heading
            )

            joinHeading = (
                self.getPoint(0).position - self.getPoint(1).position
            ).angle()
            joinPrevControl = self.getPoint(0).position + Translation2d(
                distToStart / 2.0, joinHeading
            )

            if len(self._bezierPoints) == 0:
                # We don't have any bezier points to reference
                joinSegment = PathSegment(
                    starting_pose.translation(),
                    robotNextControl,
                    joinPrevControl,
                    self.getPoint(0).position,
                ...
                return PathPlannerPath.fromPathPoints(
                    replannedPoints, self._globalConstraints, self._goalEndState
                )
            else:
                ...
                return PathPlannerPath(
                    ...
                    [
                        RotationTarget(
                            t.waypointRelativePosition + 1, t.target, t.rotateFast
                        )
                        for t in self._rotationTargets
                    ],
                    [
                        ConstraintsZone(
                            z.minWaypointPos + 1, z.maxWaypointPos + 1, z.constraints
                        )
                        for z in self._constraintZones
                    ],
                    [
                        EventMarker(m.waypointRelativePos + 1, m.command)
        ...
        for i in range(closestPointIdx, self.numPoints()):
            if (
                self.getPoint(i).distanceAlongPath
                >= self.getPoint(closestPointIdx).distanceAlongPath + closestDist
            ):
                joinAnchorIdx = i
                break
        ...
        if bezierPointIdx == len(self._bezierPoints) - 1:
            nextWaypointHeading = (
                self._bezierPoints[bezierPointIdx - 1]
                - self._bezierPoints[bezierPointIdx]
            ).angle()
        else:
            nextWaypointHeading = (
                self._bezierPoints[bezierPointIdx]
        ...
        nextWaypointPrevControl = self._bezierPoints[bezierPointIdx] + Translation2d(
            max(waypointDelta / 3.0, 0.15), nextWaypointHeading
        )
        ...
        for t in self._rotationTargets:
            if t.waypointRelativePosition >= nextWaypointIdx:
                mappedTargets.append(
                    RotationTarget(
                        t.waypointRelativePosition - nextWaypointIdx + 2,
                        t.target,
                        t.rotateFast,
                    )
                )
                ...
                mappedTargets.append(
                    RotationTarget(
                        PathPlannerPath._mapPct(pct, segment1Pct),
                        t.target,
                        t.rotateFast,
                    )
                )
    ...
    def flipPath(self) -> PathPlannerPath:
        ...
        if self._isChoreoPath:
            ...
            for state in self._choreoTrajectory.getStates():
                ...
                mirrored.targetHolonomicRotation = flipFieldRotation(
            ...
            path = PathPlannerPath(
                ...
                PathConstraints(float("inf"), float("inf"), float("inf"), float("inf")),
                GoalEndState(
                    mirroredStates[-1].velocityMps,
                    mirroredStates[-1].targetHolonomicRotation,
                    True,
                ),
            )
        ...
        newRotTargets = [
            RotationTarget(
                t.waypointRelativePosition, flipFieldRotation(t.target), t.rotateFast
            )
            for t in self._rotationTargets
        ]
        newEndState = GoalEndState(
            self._goalEndState.velocity,
        ...
        newMarkers = [
            EventMarker(m.waypointRelativePos, m.command) for m in self._eventMarkers
        ]
    ...
    def _fromJson(path_json: dict) -> PathPlannerPath:
        bezierPoints = PathPlannerPath._bezierPointsFromWaypointsJson(
            path_json["waypoints"]
        )
        ...
        rotationTargets = [
            RotationTarget.fromJson(rotJson) for rotJson in path_json["rotationTargets"]
        ]
        constraintZones = [
            ConstraintsZone.fromJson(zoneJson)
            for zoneJson in path_json["constraintZones"]
        ]
        eventMarkers = [
            EventMarker.fromJson(markerJson) for markerJson in path_json["eventMarkers"]
        ]
        previewStartingRotation = Rotation2d()
        if path_json["previewStartingState"] is not None:
            previewStartingRotation = Rotation2d.fromDegrees(
                float(path_json["previewStartingState"]["rotation"])
            )
    ...
    def _bezierPointsFromWaypointsJson(waypoints_json) -> List[Translation2d]:
        ...
        bezierPoints.append(
            PathPlannerPath._pointFromJson(firstPointJson["nextControl"])
        )
        ...
        bezierPoints.append(
            PathPlannerPath._pointFromJson(lastPointJson["prevControl"])
        )
    ...
    def _createPath(
        ...
        for s in range(numSegments):
            ...
            segmentIdx = s
            segmentRotations = [
                t.forSegmentIndex(segmentIdx)
                for t in holonomic_rotations
                if segmentIdx <= t.waypointRelativePosition <= segmentIdx + 1
            ]
            segmentZones = [
                z.forSegmentIndex(segmentIdx)
                for z in constraint_zones
                if z.overlapsRange(segmentIdx, segmentIdx + 1)
            ]

            segment = PathSegment(
                p1, p2, p3, p4, segmentRotations, segmentZones, s == numSegments - 1
            )
    ...
    def _precalcValues(self) -> None:
        if self.numPoints() > 0:
            ...
            self.getPoint(self.numPoints() - 1).rotationTarget = RotationTarget(
                -1, self._goalEndState.rotation, self._goalEndState.rotateFast
            )
