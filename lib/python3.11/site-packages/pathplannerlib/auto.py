...
class CommandUtil:
    ...
    def _sequentialGroupFromData(data_json: dict, load_choreo_paths: bool) -> Command:
        commands = [
            CommandUtil.commandFromJson(cmd_json, load_choreo_paths)
            for cmd_json in data_json["commands"]
        ]
...
class AutoBuilder:
    ...
    _pathfindToPoseCommandBuilder: Callable[
        [Pose2d, PathConstraints, float, float], Command
    ] = None
    _pathfindThenFollowPathCommandBuilder: Callable[
        [PathPlannerPath, PathConstraints, float], Command
    ] = None
    ...
    def configureHolonomic(
        ...
        if AutoBuilder._configured:
            reportError(
                "AutoBuilder has already been configured. This is likely in error.",
                True,
            )
        ...
        AutoBuilder._pathfindToPoseCommandBuilder = lambda pose, constraints, goal_end_vel, rotation_delay_distance: PathfindHolonomic(
            constraints,
            pose_supplier,
            robot_relative_speeds_supplier,
            robot_relative_output,
        ...
        AutoBuilder._pathfindThenFollowPathCommandBuilder = lambda path, constraints, rotation_delay_distance: PathfindThenFollowPathHolonomic(
            path,
            constraints,
            pose_supplier,
            robot_relative_speeds_supplier,
    ...
    def configureRamsete(
        ...
        AutoBuilder._pathfindToPoseCommandBuilder = lambda pose, constraints, goal_end_vel, rotation_delay_distance: PathfindRamsete(
            constraints,
            pose_supplier,
            robot_relative_speeds_supplier,
            robot_relative_output,
        ...
        AutoBuilder._pathfindThenFollowPathCommandBuilder = lambda path, constraints, rotation_delay_distance: PathfindThenFollowPathRamsete(
            path,
            constraints,
            pose_supplier,
            robot_relative_speeds_supplier,
    ...
    def configureLTV(
        ...
        AutoBuilder._pathfindToPoseCommandBuilder = lambda pose, constraints, goal_end_vel, rotation_delay_distance: PathfindLTV(
            constraints,
            pose_supplier,
            robot_relative_speeds_supplier,
            robot_relative_output,
        ...
        AutoBuilder._pathfindThenFollowPathCommandBuilder = lambda path, constraints, rotation_delay_distance: PathfindThenFollowPathLTV(
            path,
            constraints,
            pose_supplier,
            robot_relative_speeds_supplier,
    ...
    def pathfindToPose(
        ...
        return AutoBuilder._pathfindToPoseCommandBuilder(
            pose, constraints, goal_end_vel, rotation_delay_distance
        )
    ...
    def pathfindToPoseFlipped(
        ...
        return cmd.either(
            AutoBuilder.pathfindToPose(
                flipFieldPose(pose), constraints, goal_end_vel, rotation_delay_distance
            ),
            AutoBuilder.pathfindToPose(
                pose, constraints, goal_end_vel, rotation_delay_distance
            ),
    ...
    def pathfindThenFollowPath(
        ...
        return AutoBuilder._pathfindThenFollowPathCommandBuilder(
            goal_path, pathfinding_constraints, rotation_delay_distance
        )
    ...
    def getAutoCommandFromJson(auto_json: dict) -> Command:
        ...
        if auto_json["startingPose"] is not None:
            ...
            return cmd.sequence(
                cmd.runOnce(
                    lambda: AutoBuilder._resetPose(
                        flipFieldPose(startPose)
    ...
    def buildAuto(auto_name: str) -> Command:
        ...
        filePath = os.path.join(
            getDeployDirectory(), "pathplanner", "autos", auto_name + ".auto"
        )
...
class PathPlannerAuto(Command):
    ...
    def __init__(self, auto_name: str):
        ...
        report(
            tResourceType.kResourceType_PathPlannerAuto.value,
            PathPlannerAuto._instances,
        )
    ...
    @staticmethod
    def _pathsFromCommandJson(
        command_json: dict, choreo_paths: bool
    ) -> List[PathPlannerPath]:
        paths = []
        ...
        if cmdType == "path":
            ...
            for cmdJson in data["commands"]:
                paths.extend(
                    PathPlannerAuto._pathsFromCommandJson(cmdJson, choreo_paths)
                )
        return paths
    ...
    def getPathGroupFromAutoFile(auto_name: str) -> List[PathPlannerPath]:
        ...
        with open(filePath, "r") as f:
            ...
            return PathPlannerAuto._pathsFromCommandJson(
                auto_json["command"], choreoAuto
            )
