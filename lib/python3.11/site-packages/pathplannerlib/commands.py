...
class FollowPathCommand(Command):
    ...
    def initialize(self):
        ...
        targetHeading = (
            self._path.getPoint(1).position - self._path.getPoint(0).position
        ).angle()
        headingError = currentHeading - targetHeading
        onHeading = (
            math.hypot(currentSpeeds.vx, currentSpeeds.vy) < 0.25
            or abs(headingError.degrees()) < 30
        )

        if (
            not self._path.isChoreoPath()
            and self._replanningConfig.enableInitialReplanning
            and (
                currentPose.translation().distance(self._path.getPoint(0).position)
                > 0.25
                or not onHeading
            )
        ):
            ...
            self._generatedTrajectory = self._path.getTrajectory(
                currentSpeeds, currentPose.rotation()
            )
    ...
    def execute(self):
        ...
        if (
            not self._path.isChoreoPath()
            and self._replanningConfig.enableDynamicReplanning
        ):
            previousError = abs(self._controller.getPositionalError())
            currentError = currentPose.translation().distance(
                targetState.positionMeters
            )

            if (
                currentError
                >= self._replanningConfig.dynamicReplanningTotalErrorThreshold
                or currentError - previousError
                >= self._replanningConfig.dynamicReplanningErrorSpikeThreshold
            ):
                self._replanPath(currentPose, currentSpeeds)
                self._timer.reset()
                targetState = self._generatedTrajectory.sample(0.0)

        targetSpeeds = self._controller.calculateRobotRelativeSpeeds(
            currentPose, targetState
        )
        ...
        PPLibTelemetry.setVelocities(
            currentVel, targetState.velocityMps, currentSpeeds.omega, targetSpeeds.omega
        )
        PPLibTelemetry.setPathInaccuracy(self._controller.getPositionalError())

        self._output(targetSpeeds)

        if len(self._untriggeredEvents) > 0 and self._timer.hasElapsed(
            self._untriggeredEvents[0][0]
        ):
    ...
    def _replanPath(self, current_pose: Pose2d, current_speeds: ChassisSpeeds) -> None:
        ...
        self._generatedTrajectory = replanned.getTrajectory(
            current_speeds, current_pose.rotation()
        )
...
class PathfindingCommand(Command):
    ...
    def __init__(
        ...
        *requirements: Subsystem,
        ...
        if target_path is None and target_pose is None:
            raise ValueError(
                "Either target_path or target_pose must be specified for PathfindingCommand"
            )
        ...
        if target_path is not None:
            ...
            self._originalTargetPose = Pose2d(
                target_path.getPoint(0).position, targetRotation
            )
            ...
            self._goalEndState = GoalEndState(
                goal_end_vel, target_pose.rotation(), True
            )

        PathfindingCommand._instances += 1
        report(
            tResourceType.kResourceType_PathFindingCommand.value,
            PathfindingCommand._instances,
        )
    ...
    def execute(self):
        ...
        skipUpdates = (
            self._currentTrajectory is not None
            and currentPose.translation().distance(
                self._currentTrajectory.getEndState().positionMeters
            )
            < 2.0
        )

        if not skipUpdates and Pathfinding.isNewPathAvailable():
            self._currentPath = Pathfinding.getCurrentPath(
                self._constraints, self._goalEndState
            )

            if self._currentPath is not None:
                self._currentTrajectory = PathPlannerTrajectory(
                    self._currentPath, currentSpeeds, currentPose.rotation()
                )
                ...
                while closestState2Idx < len(self._currentTrajectory.getStates()) - 1:
                    closest2Dist = self._currentTrajectory.getState(
                        closestState2Idx
                    ).positionMeters.distance(currentPose.translation())
                    nextDist = self._currentTrajectory.getState(
                        closestState2Idx + 1
                    ).positionMeters.distance(currentPose.translation())
                ...
                currentHeading = Rotation2d(
                    fieldRelativeSpeeds.vx, fieldRelativeSpeeds.vy
                )
                headingError = currentHeading - closestState1.heading
                onHeading = (
                    math.hypot(currentSpeeds.vx, currentSpeeds.vy) < 1.0
                    or abs(headingError.degrees()) < 45
                )

                # Replan the path if our heading is off
                if onHeading or not self._replanningConfig.enableInitialReplanning:
                    d = closestState1.positionMeters.distance(
                        closestState2.positionMeters
                    )
                    t = (
                        currentPose.translation().distance(closestState1.positionMeters)
                    ) / d
                    t = min(1.0, max(0.0, t))

                    self._timeOffset = floatLerp(
                    ...
                    if (
                        ...
                        and math.hypot(currentSpeeds.vx, currentSpeeds.vy) < 0.1
                    ):
                        self._timeOffset = 0.02
                else:
                    self._currentPath = self._currentPath.replan(
        ...
        if self._currentTrajectory is not None:
            targetState = self._currentTrajectory.sample(
                self._timer.get() + self._timeOffset
            )
            ...
            if self._replanningConfig.enableDynamicReplanning:
                ...
                if (
                    ...
                    self._currentTrajectory = replanned.getTrajectory(
                        currentSpeeds, currentPose.rotation()
                    )
                    PathPlannerLogging.logActivePath(replanned)
                    PPLibTelemetry.setCurrentPath(replanned)
            ...
            # delay distance
            if (
                currentPose.translation().distance(self._startingPose.translation())
                < self._rotationDelayDistance
            ):
    ...
    def isFinished(self) -> bool:
        if self._targetPath is not None and not self._targetPath.isChoreoPath():
            ...
            currentVel = math.hypot(currentSpeeds.vx, currentSpeeds.vy)
            stoppingDistance = (currentVel**2) / (
                2 * self._constraints.maxAccelerationMpsSq
            )

            return (
                currentPose.translation().distance(self._targetPose.translation())
                <= stoppingDistance
            )

        if self._currentTrajectory is not None:
            return self._timer.hasElapsed(
                self._currentTrajectory.getTotalTimeSeconds() - self._timeOffset
            )
    ...
    def __init__(
        ...
        *requirements: Subsystem,
        ...
        super().__init__(
            ...
            *requirements,
            target_path=target_path,
            target_pose=Pose2d(target_position, Rotation2d()),
            goal_end_vel=goal_end_vel
        )
