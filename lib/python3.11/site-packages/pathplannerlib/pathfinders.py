...
class RemoteADStar(Pathfinder):
    ...
    def __init__(self):
        ...
        self._navGridJsonPub = nt.getStringTopic(
            "/PPLibCoprocessor/RemoteADStar/navGrid"
        ).publish()
        self._startPosPub = nt.getDoubleArrayTopic(
            "/PPLibCoprocessor/RemoteADStar/startPos"
        ).publish()
        self._goalPosPub = nt.getDoubleArrayTopic(
            "/PPLibCoprocessor/RemoteADStar/goalPos"
        ).publish()
        self._dynamicObstaclePub = nt.getDoubleArrayTopic(
            "/PPLibCoprocessor/RemoteADStar/dynamicObstacles"
        ).publish()

        self._pathPointsSub = nt.getDoubleArrayTopic(
            "/PPLibCoprocessor/RemoteADStar/pathPoints"
        ).subscribe([], PubSubOptions(sendAll=True, keepDuplicates=True))

        nt.addListener(
            self._pathPointsSub, EventFlags.kValueAll, self._handlePathListenerEvent
        )
    ...
    def _handlePathListenerEvent(self, event):
        ...
        for i in range(0, len(pathPointsArr) - 1, 2):
            pathPoints.append(
                PathPoint(Translation2d(pathPointsArr[i], pathPointsArr[i + 1]))
            )
...
class GridPosition:
    ...
    def __eq__(self, other):
        return (
            isinstance(other, GridPosition) and other.x == self.x and other.y == self.y
        )
...
class LocalADStar(Pathfinder):
    ...
    def setStartPosition(self, start_position: Translation2d) -> None:
        ...
        :param start_position: Start position on the field. If this is within an obstacle it will be moved to the nearest non-obstacle node.
        """
        gridPos = self._findClosestNonObstacle(
            self._getGridPos(start_position), self._requestObstacles
        )
    ...
    def setGoalPosition(self, goal_position: Translation2d) -> None:
        ...
        """
        gridPos = self._findClosestNonObstacle(
            self._getGridPos(goal_position), self._requestObstacles
        )
    ...
    def _doWork(
        ...
        do_minor: bool,
        do_major: bool,
        s_start: GridPosition,
        s_goal: GridPosition,
        real_start_pos: Translation2d,
        real_goal_pos: Translation2d,
        ...
        if do_minor:
            ...
            pathPoints = self._createPathPoints(
                pathPositions, real_start_pos, real_goal_pos, obstacles
            )
    ...
    def _extractPath(
        self, s_start: GridPosition, s_goal: GridPosition, obstacles: Set[GridPosition]
    ) -> List[GridPosition]:
        if s_goal == s_start:
            return []
    ...
    def _createPathPoints(
        ...
        bezierPoints = [
            fieldPosPath[0],
            ((fieldPosPath[1] - fieldPosPath[0]) * LocalADStar._SMOOTHING_CONTROL_PCT)
            + fieldPosPath[0],
        ]
        for i in range(1, len(fieldPosPath) - 1):
            ...
            prevControl1 = (
                (last - anchor1) * LocalADStar._SMOOTHING_CONTROL_PCT
            ) + anchor1
            nextControl1 = (
                Translation2d(controlDist, (anchor1 - prevControl1).angle()) + anchor1
            )

            prevControl2 = (
                Translation2d(controlDist, (anchor2 - next).angle()) + anchor2
            )
            nextControl2 = (
                (next - anchor2) * LocalADStar._SMOOTHING_CONTROL_PCT
            ) + anchor2
        ...
        bezierPoints.append(
            ((fieldPosPath[-2] - fieldPosPath[-1]) * LocalADStar._SMOOTHING_CONTROL_PCT)
            + fieldPosPath[-1]
        )
    ...
    def _findClosestNonObstacle(
        self, pos: GridPosition, obstacles: Set[GridPosition]
    ) -> Union[GridPosition, None]:
    ...
    def _walkable(
        self, s1: GridPosition, s2: GridPosition, obstacles: Set[GridPosition]
    ) -> bool:
        x0 = s1.x
        y0 = s1.y
        x1 = s2.x
    ...
    def _computeOrImprovePath(
        ...
        while True:
            ...
            if (
                self._comparePair(v, self._key(s_start, s_start)) >= 0
                and self._rhs[s_start] == self._g[s_start]
            ):
                break
    ...
    def _updateState(
        ...
        if s != s_goal:
            ...
            for x in self._getOpenNeighbors(s, obstacles):
                self._rhs[s] = min(
                    self._rhs[s], self._g[x] + self._cost(s, x, obstacles)
                )
    ...
    def _isCollision(
        self, s_start: GridPosition, s_end: GridPosition, obstacles: Set[GridPosition]
    ) -> bool:
    ...
    def _getOpenNeighbors(
        self, s: GridPosition, obstacles: Set[GridPosition]
    ) -> List[GridPosition]:
        ret = []
        ...
        for xMove in range(-1, 2):
            for yMove in range(-1, 2):
                ...
                if (
                    sNext not in obstacles
                    and 0 <= sNext.x < self._nodesX
                    and 0 <= sNext.y < self._nodesY
                ):
                    ret.append(sNext)
        return ret
    ...
    def _gridPosToTranslation2d(self, pos: GridPosition) -> Translation2d:
        return Translation2d(
            (pos.x * self._nodeSize) + (self._nodeSize / 2.0),
