...
class PPHolonomicDriveController(PathFollowingController):
    ...
    def __init__(
        ...
        """
        self._xController = PIDController(
        ...
        self._yController = PIDController(
            translation_constants.kP,
            translation_constants.kI,
            translation_constants.kD,
            period,
    ...
    def calculateRobotRelativeSpeeds(
        ...
        self._translationError = (
            current_pose.translation() - target_state.positionMeters
        )

        if not self._isEnabled:
            return ChassisSpeeds.fromFieldRelativeSpeeds(
                xFF, yFF, 0, current_pose.rotation()
            )
        ...
        if math.isfinite(maxAngVel):
            ...
            maxAngVelModule = (
                max(0.0, self._maxModuleSpeed - target_state.velocityMps)
                * self._mpsToRps
            )
            maxAngVel = min(angVelConstraint, maxAngVelModule)

        rotationConstraints = TrapezoidProfile.Constraints(
            maxAngVel, target_state.constraints.maxAngularAccelerationRpsSq
        )
...
class PPRamseteController(PathFollowingController, RamseteController):
    ...
    def calculateRobotRelativeSpeeds(
        ...
        :return: The next robot relative output of the path following controller
        """
        self._lastError = current_pose.translation().distance(
            target_state.positionMeters
        )

        return self.calculate(
            current_pose,
            target_state.getDifferentialPose(),
            target_state.velocityMps,
