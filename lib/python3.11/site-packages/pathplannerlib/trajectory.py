...
class State:
    ...
    def interpolate(self, end_val: State, t: float) -> State:
        ...
        lerpedState.accelerationMpsSq = floatLerp(
            self.accelerationMpsSq, end_val.accelerationMpsSq, t
        )
        lerpedState.positionMeters = translationLerp(
            self.positionMeters, end_val.positionMeters, t
        )
        lerpedState.heading = rotationLerp(self.heading, end_val.heading, t)
        lerpedState.headingAngularVelocityRps = floatLerp(
            self.headingAngularVelocityRps, end_val.headingAngularVelocityRps, t
        )
        lerpedState.curvatureRadPerMeter = floatLerp(
            self.curvatureRadPerMeter, end_val.curvatureRadPerMeter, t
        )
        lerpedState.deltaPos = floatLerp(self.deltaPos, end_val.deltaPos, t)

        if (
            self.holonomicAngularVelocityRps is not None
            and end_val.holonomicAngularVelocityRps is not None
        ):
            lerpedState.holonomicAngularVelocityRps = floatLerp(
                self.holonomicAngularVelocityRps, end_val.holonomicAngularVelocityRps, t
            )

        lerpedState.targetHolonomicRotation = rotationLerp(
            self.targetHolonomicRotation, end_val.targetHolonomicRotation, t
        )
    ...
    def reverse(self) -> State:
        ...
        reversedState.heading = Rotation2d.fromDegrees(
            inputModulus(self.heading.degrees() + 180, -180, 180)
        )
    ...
    def __init__(
        ...
        if states is not None:
            ...
            if path.isChoreoPath():
                ...
                self._states = PathPlannerTrajectory._generateStates(
                    path, starting_speeds, starting_rotation
                )
                self._eventCommands = []
                ...
                for m in path.getEventMarkers():
                    ...
                    self._eventCommands.append(
                        (self._states[pointIndex].timeSeconds, m.command)
                    )
    ...
    def _generateStates(
        ...
        for i in range(path.numPoints()):
            ...
            if i > nextRotationTargetIdx:
                prevRotationTargetDist = path.getPoint(
                    nextRotationTargetIdx
                ).distanceAlongPath
                prevRotationTargetRot = path.getPoint(
                    nextRotationTargetIdx
                ).rotationTarget.target
                nextRotationTargetIdx = PathPlannerTrajectory._getNextRotationTarget(
                    path, i
                )
            ...
            if nextTarget.rotateFast:
                ...
                t = (
                    path.getPoint(i).distanceAlongPath - prevRotationTargetDist
                ) / distanceBetweenTargets
                t = min(max(0.0, t), 1.0)
            ...
            state.curvatureRadPerMeter = (
                1.0 / curveRadius
                if (math.isfinite(curveRadius) and curveRadius != 0)
                else 0.0
            )
            ...
            if i == path.numPoints() - 1:
                ...
                state.heading = (
                    path.getPoint(i + 1).position - state.positionMeters
                ).angle()
                state.deltaPos = 0
                ...
                vMax = math.sqrt(
                    math.fabs(
                        (v0**2)
                        + (2 * constraints.maxAccelerationMpsSq * state.deltaPos)
                    )
                )
        ...
        for i in range(len(states) - 2, 1, -1):
            ...
            vMax = math.sqrt(
                math.fabs(
                    (v0**2)
                    + (2 * constraints.maxAccelerationMpsSq * states[i + 1].deltaPos)
                )
            )
