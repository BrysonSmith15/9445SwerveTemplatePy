...
class CANBus:
    ...
    def is_network_fd(canbus: str) -> bool:
        ...
        :rtype: bool
        """
        return Native.instance().c_ctre_phoenix6_platform_canbus_is_network_fd(
            ctypes.c_char_p(bytes(canbus, "utf-8"))
        )
    ...
    def get_status(canbus: str) -> CANBusStatus:
        ...
        rec = ctypes.c_uint32()
        tec = ctypes.c_uint32()
        err = Native.instance().c_ctre_phoenix6_platform_canbus_get_status(
            ctypes.byref(bus_util_perc),
            ctypes.byref(bus_off_cnt),
            ctypes.byref(tx_full_cnt),
            ctypes.byref(rec),
            ctypes.byref(tec),
            canbus_cstr,
            True,
