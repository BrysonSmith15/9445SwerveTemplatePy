...
class DifferentialFollower:
    ...
    def _send_request(
        ...
        """
        return StatusCode(
            Native.instance().c_ctre_phoenix6_RequestControlDifferentialFollower(
                ctypes.c_char_p(bytes(network, "utf-8")),
                device_hash,
                self.update_freq_hz,
                cancel_other_requests,
                self.master_id,
                self.oppose_master_direction,
            )
        )
    ...
    def with_oppose_master_direction(
        self, new_oppose_master_direction: bool
