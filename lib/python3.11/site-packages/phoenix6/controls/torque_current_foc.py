...
class TorqueCurrentFOC:
    ...
    def _send_request(
        ...
        """
        return StatusCode(
            Native.instance().c_ctre_phoenix6_RequestControlTorqueCurrentFOC(
                ctypes.c_char_p(bytes(network, "utf-8")),
                device_hash,
                self.update_freq_hz,
                cancel_other_requests,
                self.output,
                self.max_abs_duty_cycle,
                self.deadband,
    ...
    def with_max_abs_duty_cycle(
        self, new_max_abs_duty_cycle: float
    ) -> "TorqueCurrentFOC":
        """
