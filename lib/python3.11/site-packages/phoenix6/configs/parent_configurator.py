...
class ParentConfigurator:
    ...
    def _set_configs_private(
        ...
        status = StatusCode(
            Native.instance().c_ctre_phoenix6_set_configs(
                0,
                ctypes.c_char_p(
                    bytes(self.__device_identifier.network, encoding="utf-8")
                ),
    ...
    def _get_configs_private(self, timeout_seconds: second) -> (StatusCode, str):
        ...
        status = StatusCode(
            Native.instance().c_ctre_phoenix6_get_configs(
                0,
                ctypes.c_char_p(
                    bytes(self.__device_identifier.network, encoding="utf-8")
                ),
