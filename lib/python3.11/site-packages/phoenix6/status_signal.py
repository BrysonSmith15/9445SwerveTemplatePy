...
class StatusSignal(BaseStatusSignal, Generic[T]):
    ...
    def __init__(
        ...
        if error is None:
            ...
            self.__basic_type_map = (
                generator() if self.__contains_underlying_types else None
            )
    ...
    def __refresh_mappable(self, wait_for_signal: bool, timeout_seconds: second):
        ...
        to_get = ReturnValues()
        error = StatusCode(
            Native.instance().c_ctre_phoenix6_get_signal(
                1,
                ctypes.byref(to_send),
                ctypes.byref(to_get),
                ctypes.c_char_p(bytes(self._identifier.network, "utf-8")),
                1 if wait_for_signal else 0,
                timeout_seconds,
            )
        )
    ...
    def __refresh_nonmappable(self, wait_for_signal: bool, timeout_seconds: second):
        ...
        self._all_timestamps.update(
            to_get.swtimestampseconds,
            TimestampSource.System,
            True,
            to_get.hwtimestampseconds,
            TimestampSource.CANivore,
            True,
            to_get.ecutimestampseconds,
            TimestampSource.Device,
            to_get.ecutimestampseconds != 0.0,
        )

    def __update_value(
        self, wait_for_signal: bool, timeout: second, report_error: bool
    ):
    ...
    def wait_for_update(
        self, timeout_seconds: second, report_error: bool = True
    ) -> "StatusSignal[T]":
        """
    ...
    def set_update_frequency(
        self, frequency_hz: hertz, timeout_seconds: second = 0.050
    ) -> StatusCode:
        """
        ...
        """
        if self.__contains_underlying_types:
            return next(iter(self.__basic_type_map.values())).set_update_frequency(
                frequency_hz, timeout_seconds
            )
        else:
            network = ctypes.c_char_p(bytes(self._identifier.network, "utf-8"))
            return StatusCode(
                Native.instance().c_ctre_phoenix6_SetUpdateFrequency(
                    0,
                    network,
                    self._identifier.device_hash,
                    self._spn,
                    frequency_hz,
                    timeout_seconds,
                )
    ...
    def get_applied_update_frequency(self) -> hertz:
        ...
        if self.__contains_underlying_types:
            return next(
                iter(self.__basic_type_map.values())
            ).get_applied_update_frequency()
        else:
            network = ctypes.c_char_p(bytes(self._identifier.network, "utf-8"))
            return Native.instance().c_ctre_phoenix6_GetUpdateFrequency(
                network, self._identifier.device_hash, self._spn
            )
