...
class BaseStatusSignal(ABC):
    ...
    @staticmethod
    def wait_for_all(
        timeout_seconds: second, *signals: "BaseStatusSignal | list[BaseStatusSignal]"
    ) -> StatusCode:
        """
        ...
        :rtype: StatusCode"""
        return BaseStatusSignal.__wait_for_all_impl(
            "BaseStatusSignal.wait_for_all",
            timeout_seconds,
            BaseStatusSignal.__flatten_signals(*signals),
        )

    @staticmethod
    def refresh_all(
        *signals: "BaseStatusSignal | list[BaseStatusSignal]",
    ) -> StatusCode:
        """
    ...
    @staticmethod
    def get_latency_compensated_value(
        signal: "StatusSignal[float]",
        signal_slope: "StatusSignal[float]",
        max_latency_seconds: second = 0.300,
    ) -> float:
        """
    ...
    @staticmethod
    def set_update_frequency_for_all(
        frequency_hz: hertz, *signals: "BaseStatusSignal | list[BaseStatusSignal]"
    ) -> StatusCode:
        """
        ...
        return StatusCode(
            Native.instance().c_ctre_phoenix6_SetUpdateFrequencyForAll(
                0, sigs, count, frequency_hz, 0.050
            )
        )
    ...
    @staticmethod
    def __wait_for_all_impl(
        location: str, timeout_seconds: second, signals: list["BaseStatusSignal"]
    ) -> StatusCode:
        count = len(signals)
        if count < 1:
        ...
        for signal, ret in zip(signals, rets):
            ...
            signal._all_timestamps.update(
                ret.swtimestampseconds,
                TimestampSource.System,
                True,
                ret.hwtimestampseconds,
                TimestampSource.CANivore,
                True,
                ret.ecutimestampseconds,
                TimestampSource.Device,
                ret.ecutimestampseconds != 0.0,
            )
    ...
    @staticmethod
    def __flatten_signals(
        *signals: "BaseStatusSignal | list[BaseStatusSignal]",
    ) -> list["BaseStatusSignal"]:
