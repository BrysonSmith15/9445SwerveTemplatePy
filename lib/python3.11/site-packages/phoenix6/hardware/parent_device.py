...
class ParentDevice:
    ...
    def __init__(self, device_id: int, model: str, canbus: str):
        ...
        self.__compliancy = self._common_lookup(
            SpnValue.COMPLIANCY_VERSION.value, 0, None, "Compliancy", False, int
        )
        self.__reset_signal = self._common_lookup(
            SpnValue.STARTUP_RESET_FLAGS.value, 0, None, "ResetFlags", False, int
        )
    ...
    def optimize_bus_utilization(
        self, optimized_freq_hz: hertz = 0.0, timeout_seconds: second = 0.05
    ) -> StatusCode:
        """
        ...
        return StatusCode(
            Native.instance().c_ctre_phoenix6_OptimizeUpdateFrequencies(
                0, network, self.device_hash, optimized_freq_hz, timeout_seconds
            )
        )

    @staticmethod
    def optimize_bus_utilization_for_all(
        *devices: "ParentDevice | list[ParentDevice]", optimized_freq_hz: hertz = 0.0
    ) -> StatusCode:
        """
    ...
    def __report_if_too_old(self):
        ...
        if current_time >= self.__time_to_refresh_version:
            ...
            if code.is_ok():
                ...
                code is StatusCode.FIRMWARE_TOO_OLD or code is StatusCode.API_TOO_OLD
            ) or delta_time_sec >= 3.0:
                # report error
    ...
    def _set_control_private(self, request: SupportsSendRequest) -> StatusCode:
        ...
        if not self.__version_status.is_ok() and self.__compliancy.status.is_ok():
            ...
            self.__control_request = ParentDevice.__empty_control
            ParentDevice.__empty_control._send_request(
                self.network, self.device_hash, True
            )
            ...
            status = request._send_request(
                self.network, self.device_hash, cancel_other_requests
            )
    ...
    def _common_lookup(
        ...
        spn: int,
        map_iter: int,
        generator: Callable[[], dict[int, StatusSignal]],
        signal_name: str,
        report_on_construction: bool,
        signal_type: type[T],
    ) -> StatusSignal[T]:
        ...
        if total_hash in self.__signal_values:
            ...
            # Insert into map
            if map_iter == 0:
                self.__signal_values[total_hash] = StatusSignal(
                    None,
    ...
    @staticmethod
    def __flatten_devices(
        *devices: "ParentDevice | list[ParentDevice]",
    ) -> list["ParentDevice"]:
